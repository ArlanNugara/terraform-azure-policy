module "guest_attestation_extension_should_be_installed_on_supported_linux_virtual_machines_policy" {
  source                         = "./modules/generic_policy_module"
  policy_definition_name         = "${var.org_prefix}Guest Attestation extension should be installed on supported Linux virtual machines"
  ...
  policy_definition_rule = jsonencode(
    {
      ...
      "then" : {
        "effect" : "${var.guest_attestation_extension_should_be_installed_on_supported_linux_virtual_machines_policy_effect}",
        "details" : {
